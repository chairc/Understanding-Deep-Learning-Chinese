...
本书的结构沿用了引言的结构。第 2 章到第 9 章介绍了监督学习路线，我们描述了浅层和深层神经网络，讨论了如何训练，以及如何衡量和提高它们的性能。第 10 章到第 13 章描述了深度神经网络的常见架构变体，包括卷积网络、残差连接和Transformer。这些架构用于监督学习、无监督学习和强化学习。

第 14 章到第 18 章使用深度神经网络处理无监督学习，我们分别用一章来介绍四种现代深度生成模型：*生成对抗网络*（*generative adversarial networks*）、*变分自动编码器*（*variational autoencoders*）、*标准化流*（*normalizing flows*）和*扩散模型*（*diffusion models*）。第 19 章是对深度强化学习的简要介绍，这是一个很容易为自己的书辩护的主题，因此处理必然是表面的。但是这种处理方式旨在为不熟悉该领域的读者提供一个良好的起点。

尽管书名如此，但深度学习的某些方面仍然知之甚少。第 20 章提出了一些基本问题，为什么深度网络如此容易训练？为什么他们概括得这么好？为什么他们需要这么多参数？他们需要很深吗？一路上，我们探索了一些意想不到的现象，例如损失函数的结构、双重下降、grokking 和彩票。本书以第 21 章结尾，讨论了伦理学和深度学习。

## 1.6 其它书目

1. 《Deep Learning》 (Goodfellow et al., 2016)  [16]
2. 《Probabilistic Machine Learning》 (Murphy, 2022, 2023)  [17]
3. 《Pattern Recognition and Machine Learning》 (Bishop, 2006)   [18]
4. 《Computer Vision: Models, Learning, and Inference》 (Prince, 2012)  [19]
5. 《Probabilistic Graphical Models: Principles and Techniques》 (Koller & Friedman, 2009) [20]
6. 《Gaussian Processes for Machine Learning》 (Williams & Rasmussen, 2006)  [21]
7. 《Mathematics for Machine Learning》 (Deisenroth et al., 2020)  [22]
8. 《Dive into Deep Learning》 (Zhang et al., 2023)  [23]
9. 《Computer Vision: A Deep Learning Approach》 (Torralba et al., forthcoming)  [24]
10. 《Graph Representation Learning》 (Hamilton, 2020)  [25]
11. 《Reinforcement Learning: An Introduction》 (Sutton & Barto, 2018)  [26]
12. 《Foundations of Deep Reinforcement Learning》 (Graesser & Keng, 2019)  [27]
13. 《Computer vision: Algorithms and applications》(Szeliski, 2022) [28]

## 1.7 如何阅读这本书

本书剩余的大部分章节均包含正文、注释部分和习题集。正文的主体旨在自成一体，无需借助本章的其他部分即可阅读。尽可能多地将数学背景纳入正文。然而对于会分散论证主线的较大主题，材料附在附录中，并在空白处提供参考。本书中的大多数符号是标准的，然而一些约定使用不那么广泛，鼓励读者在继续之前查阅附录 A。

正文主体包括许多深度学习模型和结果的新颖插图和可视化。我一直在努力为现有想法提供新的解释，而不仅仅是策划他人的工作。深度学习是一个新领域，有时人们对现象知之甚少。我试图弄清楚哪里是这种情况，以及何时应该谨慎对待我的解释。

参考文献仅在描述结果的章节中出现。它们可以在本章末尾的注释部分找到。我一般不尊重正文中的历史先例，如果一个现在的技术的祖宗已经没有用了，那我就不提了。然而，该领域的历史发展在注释部分有所描述，希望公平地分配功劳。这些注释被组织成段落，并为进一步阅读提供了指导。它们应该帮助读者在子领域内定位自己，并了解它与机器学习的其他部分的关系。注释不如正文独立，根据您的背景知识水平和兴趣，您可能会发现这些部分或多或少有用。

每章都有一些相关的问题。它们在正文的空白处在应该尝试的地方被引用。正如 George Pólya 所说，“你看，数学不是一项观赏性运动。”他说的很对，我强烈建议你一边做一边尝试这些问题。在某些情况下，它们提供的见解将帮助您理解正文。在相关网站上提供答案的问题用星号表示。此外，网站上还提供了有助于理解本书思想的 Python 笔记本，这些笔记本也在本书的页边空白处进行了引用。如果你能真的感到生疏，那现在可能值得通过笔记本上的背景数学学习。

不幸的是，人工智能研究的步伐使得本书不可避免地会不断发展。如果您发现难以理解的部分、明显的遗漏或看似无关的部分，请通过相关网站与我们联系。我们一起努力，使下一版做的更好。
...
[27] Graesser, L., & Keng, W. L. (2019). Foundations of deep reinforcement learning. Addison-Wesley Professional.

[28] Szeliski, R. (2022). Computer vision: Algorithms and applications, 2nd Edition. Springer.
